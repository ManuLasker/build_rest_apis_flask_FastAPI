...
class UserRouter:
    ...
    def delete(user_id:int, db: Session = Depends(get_db)):
        ...
        if db_user:
            ...
            return db_user
        raise HTTPException(
            status_code=404, detail={"message": f"User with user_id '{user_id}' not found"}
        )
    ...
    def register(new_user: UserCreate, db: Session = Depends(get_db)):
        ...
        if db_user:
            raise HTTPException(status_code=400, 
                                detail={"message": f"User with username {new_user.username} already exists!"})
...
class UserAuthenticationRouter:
    ...
    def authenticate(info_user: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
        ...
        raise HTTPException(
            status_code=401, detail={"message": "invalid credentials"}
        )
